...
namespace inputNS
{
	const int KEYS_ARRAY_LEN = 256;		// size of key arrays
										// what values for clear(), but flag
...
const DWORD GAMEPAD_THUMBSTICK_DEADZONE = (DWORD)(0.20f * 0X7FFFF);		// default to 20% of range as deadzone
const DWORD GAMEPAD_TRIGGER_DEADZONE = 30;								// trigger range 0 - 255
const DWORD MAX_CONTROLLERS = 4;										// Maximum number of controllers supposed by XInput
...
public:
	...
	// Return true if any key was pressed in the mot recent frame
	// Key presses are erased at the end of each frame
	bool anyKeyPressed() const;
	// Clear the specified key press
	void clearKeyPress(UCHAR vkey);
	// Clear specified input buffers where what is any combination of KEYS_DOWN, KEYS_PRESSED, MOUSE, TEXT_IN, KEYS_MOUSE_TEXT
